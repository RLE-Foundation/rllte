...
def _non_linear_scaling(profile_dict: Dict[str, List],
                        tau_list: np.ndarray,
                        xticklabels: Optional[List] = None,
                        num_points: int = 5,
                        log_base: float = 2) -> Tuple:
    ...
    Args:
        ...
        num_points (int): If `xticklabels` are not passed, then specifices the number of indices to be generated on a log scale.
...
def plot_performance_profile(
        ...
        **kwargs) -> None:
        ...
        return fig, _annotate_and_decorate_axis(ax,
                                                xticks=xticks,
                                                yticks=yticks,

...
for seed in {1..3}; do
    ...
    for reward in "${intrinsic_rewards[@]}"; do
        ...
        python src/ppo.py --env_id=MiniHack-MultiRoom-N4-v0 --intrinsic_reward=${reward} --seed=${seed} --obs_rms

        # Run without --obs_rms
        echo "Running with seed=${seed}, intrinsic_reward=${reward}, and without obs_rms"
        python src/ppo.py --env_id=MiniHack-MultiRoom-N4-v0 --intrinsic_reward=${reward} --seed=${seed}
    done
done

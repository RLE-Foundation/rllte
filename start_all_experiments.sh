...
intrinsic_rewards=("extrinsic" "pseudocounts" "icm" "rnd" "ngu" "ride" "re3" "rise" "revd" "e3b" "disagreement")
...
for seed in {1..3}; do
    ...
    for reward in "${intrinsic_rewards[@]}"; do
        ...
        python src/ppo.py --env_id=MiniGrid-FourRooms-v0 --intrinsic_reward=${reward} --seed=${seed} --obs_rms

        # Run without --obs_rms
        echo "Running with seed=${seed}, intrinsic_reward=${reward}, and without obs_rms"
        python src/ppo.py --env_id=MiniGrid-FourRooms-v0 --intrinsic_reward=${reward} --seed=${seed}
    done
done
